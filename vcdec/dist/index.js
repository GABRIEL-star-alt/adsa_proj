...
import { check } from "./vc/check.js";
const program = new Command();
program
    .name("vcdec")
    .version("1.1.0-alpha")
    .description(figlet.textSync("vcdec") + "\n  An IPFS based version control system with static file hosting features");
program.command("init <ipfs_node_url>").description("Initialize a new vcdec repository");
program.command("add [file_path]").description("Add a file to the vcdec repository");
program.command("commit <message>").description("Commit changes to the vcdec repository");
program.command("log").description("View the commit history of the current branch");
program.command("branch").description("List all branches in the vcdec repository");
program.command("jump <branch>").description("Switch between branches");
program.command("check <CID>").description("...")
